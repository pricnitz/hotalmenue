...
export default function WaiterDashboard() {
  ...
  // Mobile Responsiveness Tab View
  const [mobileTab, setMobileTab] = useState("pending"); // "pending", "ready", "cooking"
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-zinc-950 text-slate-800 dark:text-slate-100 flex flex-col font-sans">
      ...
      {/* Navbar */}
      <header className="bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-slate-800 px-4 sm:px-6 py-3.5 flex items-center justify-between shadow-xs sticky top-0 z-30">
        <div className="flex items-center gap-2.5">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-tr from-brand-500 to-amber-500 text-white shadow-xs flex-none">
          ...
          <div className="text-left">
            <span className="font-extrabold text-sm sm:text-base tracking-tight text-slate-900 dark:text-white flex items-center gap-1.5">
              Waiter Console
              <span className="text-[9px] bg-slate-100 dark:bg-zinc-800 text-slate-500 dark:text-slate-400 font-extrabold px-1.5 py-0.5 rounded border border-slate-200/50 uppercase">WAITSTAFF</span>
            </span>
            <span className="text-[10px] text-slate-400 font-semibold block sm:hidden">Fast Mobile POS</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <button
            ...
            className="rounded-xl bg-brand-500 hover:bg-brand-600 px-3 sm:px-4 py-2 text-xs font-extrabold text-white shadow-sm transition-all active:scale-95 cursor-pointer flex items-center gap-1"
          >
          ...
          <button
            ...
            disabled={loggingOut}
            className="rounded-xl bg-slate-900 dark:bg-slate-100 dark:text-slate-900 px-3 sm:px-4 py-2 text-xs font-bold text-white shadow-sm disabled:opacity-50 transition-all active:scale-95 cursor-pointer"
          >
            {loggingOut ? "..." : "Logout"}
          </button>
        </div>
      ...
      {/* Main Container */}
      <main className="flex-grow p-3 sm:p-6 max-w-7xl mx-auto w-full space-y-4 sm:space-y-6 pb-20 lg:pb-6">

        {/* Mobile Segmented Navigation Control (< lg screens) */}
        <div className="lg:hidden bg-slate-200/80 dark:bg-zinc-900 p-1 rounded-2xl grid grid-cols-3 gap-1 text-center font-extrabold text-xs">
          <button
            type="button"
          ...
          >
            ...
            {pendingOrders.length > 0 && (
              <span className="bg-brand-500 text-white text-[10px] px-1.5 py-0.2 rounded-full">
          ...
          >
            ...
            {readyOrders.length > 0 && (
              <span className="bg-white text-emerald-600 text-[10px] px-1.5 py-0.2 rounded-full font-black animate-pulse">
                {readyOrders.length}
              </span>
            )}
          </button>
          ...
          >
            ...
            {cookingOrders.length > 0 && (
              <span className="bg-slate-200 dark:bg-zinc-800 text-slate-700 dark:text-slate-300 text-[10px] px-1.5 py-0.2 rounded-full">
        ...
        {readyOrders.length > 0 && (
          <div className="bg-emerald-500 text-white border border-emerald-400 p-3.5 sm:p-4 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-lg shadow-emerald-500/20 animate-pulse text-left w-full">
            <div className="flex items-center gap-3">
              <span className="text-2xl leading-none flex-none">🛎️</span>
              <div>
                <h4 className="font-black text-xs sm:text-sm">Dishes Ready for Table Runners!</h4>
            ...
            <div className="flex flex-wrap gap-2 w-full sm:w-auto">
              {readyOrders.map(o => (
                <button
                  ...
                  className="flex-1 sm:flex-none bg-white text-emerald-600 hover:bg-emerald-50 rounded-xl px-3 py-2 text-xs font-extrabold shadow-xs active:scale-95 transition-all cursor-pointer text-center"
                >
                  Deliver {o.tableId} ✓
                </button>
              ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-start">
          ...
          {/* Left Column: Pending approvals (Visible on Desktop OR when mobileTab === 'pending') */}
          <section className={`lg:col-span-7 space-y-4 sm:space-y-6 text-left ${mobileTab === "pending" ? "block" : "hidden lg:block"}`}>
            <div className="space-y-0.5">
              <h2 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white">New Table Requests ({pendingOrders.length})</h2>
            ...
            {loading ? (
              ...
              <div className="space-y-3 sm:space-y-4">
                {pendingOrders.map((order) => (
                  <div
                    key={order._id}
                    className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-slate-800 p-4 sm:p-5 rounded-2xl sm:rounded-3xl shadow-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 relative overflow-hidden transition-all"
                  >
                    <div className="space-y-2 w-full sm:w-auto">
                      <div className="flex items-center justify-between sm:justify-start gap-2">
                        <span className="text-xs font-extrabold uppercase bg-brand-500/10 text-brand-500 border border-brand-500/20 px-2.5 py-0.5 rounded-md">
                          {order.tableId}
                        </span>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {order.items.map((i, idx) => (
                          <span key={idx} className="bg-slate-50 dark:bg-zinc-950 border border-slate-200/50 dark:border-slate-800 px-2 py-1 rounded-lg text-xs font-medium text-slate-700 dark:text-slate-300">
                            {i.qty}x {i.name}
                          </span>
                        ))}
                      </div>
                      ...
                      {order.customerNotes && (
                        <p className="text-[10px] text-slate-400 leading-relaxed font-semibold italic bg-slate-50 dark:bg-zinc-950 p-2 rounded-lg border border-slate-100 dark:border-slate-800">Note: "{order.customerNotes}"</p>
                      )}
                    </div>

                    <div className="flex gap-2 w-full sm:w-auto flex-none pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800">
                      <button
                        onClick={() => handleRejectOrder(order._id)}
                        className="flex-1 sm:flex-none border border-slate-250 dark:border-slate-800 hover:bg-red-50 hover:text-red-600 text-slate-500 rounded-xl px-3.5 py-2.5 text-xs font-bold transition-all cursor-pointer text-center"
                      >
                        Reject
                      </button>
                      <button
                        onClick={() => openEditDrawer(order)}
                        className="flex-1 sm:flex-none bg-brand-500 hover:bg-brand-600 text-white rounded-xl px-4 py-2.5 text-xs font-bold shadow-md active:scale-95 transition-all cursor-pointer text-center"
                      >
                        Review & Approve
                      </button>
                    </div>
              ...
              <div className="py-12 sm:py-16 bg-white dark:bg-zinc-900 border border-slate-200/50 dark:border-slate-800 rounded-2xl sm:rounded-3xl text-center">
                <CheckCircleIcon className="w-8 h-8 mx-auto text-emerald-500 opacity-60 mb-2 animate-bounce" />
                <p className="text-slate-400 text-xs sm:text-sm font-medium">All new diner orders are approved!</p>
              </div>
            )}
          </section>

          {/* Right Column: Active queues (Visible on Desktop OR when mobileTab === 'ready' / 'cooking') */}
          <section className={`lg:col-span-5 space-y-6 text-left ${mobileTab !== "pending" ? "block" : "hidden lg:block"}`}>
            
            {/* Ready to Deliver Column */}
            <div className={`space-y-3 sm:space-y-4 ${mobileTab === "ready" || mobileTab === "all" ? "block" : "hidden lg:block"}`}>
              <div className="space-y-0.5">
                <h2 className="text-xs sm:text-sm uppercase tracking-widest font-black text-emerald-600 dark:text-emerald-400">🛎️ Ready to Deliver ({readyOrders.length})</h2>
              ...
              {readyOrders.length > 0 ? (
                <div className="space-y-3">
                  {readyOrders.map((order) => (
                    <div key={order._id} className="bg-white dark:bg-zinc-900 border-2 border-emerald-500/40 p-4 rounded-2xl flex items-center justify-between gap-4 shadow-xs">
                      ...
                      <button
                        ...
                        className="bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl px-4 py-2.5 text-xs font-bold shadow-xs active:scale-95 cursor-pointer flex-none"
                      >
                        Serve ✓
                      </button>
                    </div>
            ...
            {/* Kitchen Queue */}
            <div className={`space-y-3 sm:space-y-4 pt-2 sm:pt-4 ${mobileTab === "cooking" || mobileTab === "all" ? "block" : "hidden lg:block"}`}>
              <div className="space-y-0.5">
                <h2 className="text-xs sm:text-sm uppercase tracking-widest font-black text-slate-400">👨‍🍳 Cooking Queue ({cookingOrders.length})</h2>
                <p className="text-[10px] text-slate-400">Tickets currently in preparation</p>
              </div>

              {cookingOrders.length > 0 ? (
                <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl overflow-hidden shadow-xs">
                  <div className="divide-y divide-slate-100 dark:divide-slate-800">
                    {cookingOrders.map((order) => (
                      <div key={order._id} className="p-3.5 sm:p-4 flex items-center justify-between gap-4 text-xs font-medium">
                        ...
                        <span className="font-black text-slate-900 dark:text-white flex-none">{getCurrencySymbol()}{order.total.toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                </div>
      ...
      {/* Sticky Mobile Bottom Navigation Bar (< lg screens) */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-slate-950/95 backdrop-blur-md border-t border-slate-800 p-2 flex items-center justify-around text-slate-400 font-extrabold text-[10px]">
        <button
          type="button"
          onClick={() => setMobileTab("pending")}
          className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all cursor-pointer ${
            mobileTab === "pending" ? "text-brand-400 bg-brand-500/10" : "hover:text-white"
          }`}
        >
          <span className="text-base leading-none">📋</span>
          <span>Requests ({pendingOrders.length})</span>
        </button>
        ...
        <button
          ...
          onClick={() => setMobileTab("ready")}
          className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all cursor-pointer ${
            mobileTab === "ready" ? "text-emerald-400 bg-emerald-500/10" : "hover:text-white"
          }`}
        >
        ...
        <button
          ...
          onClick={() => setMobileTab("cooking")}
          className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all cursor-pointer ${
            mobileTab === "cooking" ? "text-white bg-slate-800" : "hover:text-white"
          }`}
        >
          <span className="text-base leading-none">👨‍🍳</span>
        ...
        <button
          ...
          className="flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl bg-brand-500 text-white font-extrabold shadow-md active:scale-95 transition-all cursor-pointer"
        >
          <span className="text-base leading-none">+</span>
          <span>New Order</span>
        </button>
      ...
      {editingOrder && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-xs p-0 sm:p-4 animate-fade-in">
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-slate-800 rounded-t-3xl sm:rounded-3xl p-5 sm:p-8 max-w-xl w-full max-h-[92vh] sm:max-h-[90vh] shadow-2xl space-y-5 text-left flex flex-col justify-between">
      ...
      {/* Place Order Modal */}
      {showPlaceOrderModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xs p-0 sm:p-4 animate-scale-in">
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-slate-850 p-4 sm:p-6 sm:rounded-3xl shadow-xl w-full h-full sm:h-auto max-w-4xl flex flex-col max-h-none sm:max-h-[92vh] text-left">
            
            {/* Modal Header */}
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3 mb-3 sm:mb-4 flex-none">
              <div>
                <h3 className="font-extrabold text-sm sm:text-base text-slate-900 dark:text-white">🛎️ Place Order on Behalf of Customer</h3>
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-base font-black cursor-pointer p-1"
              >
                ✕
              </button>
            </div>
            ...
            {/* Modal Core Layout */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 overflow-y-auto sm:overflow-hidden flex-grow">
              
              {/* Left & Middle Column (8 cols): Table, Search & Two-Column Category Menu picker */}
              <div className="lg:col-span-8 flex flex-col space-y-3 sm:space-y-4 overflow-hidden">
                
                {/* Table Select + Search Bar */}
                <div className="flex gap-2 sm:gap-3 items-center flex-none bg-slate-50 dark:bg-zinc-955 p-2.5 sm:p-3 rounded-2xl border border-slate-200/50 dark:border-slate-850">
                  <div className="w-2/5 sm:w-1/3">
                    ...
                    <select
                      ...
                      className="w-full text-xs font-bold px-2.5 py-2 border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-zinc-900 focus:outline-none focus:ring-1 focus:ring-brand-500 text-slate-800 dark:text-slate-200"
                    >
                  ...
                  <div className="w-3/5 sm:w-2/3">
                    ...
                    <input
                      ...
                      className="w-full text-xs font-semibold px-3 py-2 border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-zinc-900 focus:outline-none focus:ring-1 focus:ring-brand-500 text-slate-800 dark:text-slate-200"
                    />
                  </div>
                </div>

                {/* Two-Column Category selector + Dishes List */}
                <div className="grid grid-cols-12 gap-3 sm:gap-4 flex-grow overflow-hidden">
                  
                  {/* Category Navigation (Horizontal scroll on mobile, Vertical sidebar on desktop) */}
                  <div className="col-span-12 sm:col-span-4 bg-slate-50 dark:bg-zinc-955 border border-slate-200/50 dark:border-slate-850 p-2 rounded-2xl flex sm:flex-col overflow-x-auto sm:overflow-y-auto space-x-2 sm:space-x-0 sm:space-y-1.5 no-scrollbar flex-none sm:flex-auto">
                    <button
                      type="button"
                      onClick={() => setActiveCategory("All")}
                      className={`w-auto sm:w-full flex-none text-left px-3 py-2 sm:py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer whitespace-nowrap ${
                        activeCategory === "All"
                    ...
                    {categories.map((cat) => (
                      <button
                        ...
                        className={`w-auto sm:w-full flex-none text-left px-3 py-2 sm:py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 sm:gap-2 whitespace-nowrap ${
                  ...
                  <div className="col-span-12 sm:col-span-8 overflow-y-auto pr-1 no-scrollbar max-h-[320px] sm:max-h-none">
